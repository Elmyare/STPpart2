...
class ControlPanel:
    ...
    def _setup_ui(self):
        ...
        # Только кнопки памяти (без стрелок)
        memory_frame = tk.Frame(self._root)
        memory_frame.pack(fill=tk.X, padx=10, pady=5)
        
        # Добавляем кнопки Backspace и Delete рядом с памятью
        tk.Button(memory_frame, text="⌫", width=5, command=lambda: self._on_button_click('Backspace')).pack(side=tk.LEFT)
        tk.Button(memory_frame, text="Del", width=5, command=lambda: self._on_button_click('Delete')).pack(side=tk.LEFT)
        
        tk.Button(memory_frame, text="MC", width=5, command=self._memory_clear).pack(side=tk.RIGHT)
        tk.Button(memory_frame, text="MR", width=5, command=self._memory_recall).pack(side=tk.RIGHT)
        tk.Button(memory_frame, text="MS", width=5, command=self._memory_store).pack(side=tk.RIGHT)
        tk.Button(memory_frame, text="M+", width=5, command=self._memory_add).pack(side=tk.RIGHT)
        
        # Основные кнопки (без изменений)
    ...
    def _handle_key_press(self, event):
        ...
        if event.keysym == 'BackSpace':
            self._on_button_click('Backspace')
            return "break"
        elif event.keysym == 'Delete':
            self._on_button_click('Delete')
            return "break"
        elif event.keysym == 'Left':
            self._device._editor.move_cursor_left()
            self._display.icursor(self._device._editor._cursor_pos)
            return "break"
        elif event.keysym == 'Right':
            self._device._editor.move_cursor_right()
            self._display.icursor(self._device._editor._cursor_pos)
            return "break"
        ...
        if event.char in allowed_chars:
            try:
                ...
                # Обновляем позицию курсора после ввода
                self._display.icursor(self._device._editor._cursor_pos)
